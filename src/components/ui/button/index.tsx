...
const buttonStyles = cva(
  'group relative cursor-pointer font-semibold transition duration-300 disabled:opacity-50',
  {
    variants: {
      variant: {
        cta: 'w-full max-w-[370px] bg-(--color-blue) text-white py-4  text-lg hover:shadow-2xl hover:translate-y-[-2px] group flex justify-center items-center gap-4 rounded-[5px] transition-all duration-300 ease-in-out font-suisse',
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  return (
    ...
    >
      {children}
      {variant === 'cta' && (
        <div className="h-[10px] w-6 origin-left transition duration-300 ease-in-out group-hover:scale-x-[120%]">
          <svg
            viewBox="0 0 23 10"
            fill="none"
